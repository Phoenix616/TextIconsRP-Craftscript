/* "char" CraftScript by WolfieMario. Updated to WE6 by Phoenix616
 *
 ...
 *
 *
 * Usage: select command blocks and/or signs containing special text
 * you wish to replace with characters from custom fontsheets. The
 * text to be replaced is the name of the character to use, between
 * braces {}. Example: "{wooden sword}" will become a Wooden Sword
 * icon. The text is case-insensitive; "{Wooden Sword}" will also
 * work.
 * 
 ...
 * change the value of unicodeOffset accordingly (for example, to
...
var usageText = "[escape]";
var localSession = context.getSession();
var editSession = context.remember();

var unicodeOffset = 0x9000;
var characterNames = new ArrayList([
...
/**
 * Reads parameters and initializes global variables.
 * @returns {Boolean} - Whether or not initialization succeeded.
 */
function init()
{
	context.checkArgs(0, 1, usageText);
	
	if(argv.length > 1 && (argv[1].equals("usage") || argv[1].equals("help")))
	{
		player.printError("Usage: " + usageText);
		
		if(argv[1].equals("help"))
			player.print("Select command blocks and/or signs containing text to replace with custom characters.\n" +
					"Wrap character names in brackets, such as \"{wooden sword}\".\n" +
					"Add the parameter \"escape\" to have custom characters preceded by the white format code and " +
					"followed by the black format code.");
		
		return false;
	}
	
	escape = false;
	if(argv.length > 1 && argv[1].equals("escape"))
		escape = true;
...
/**
 * Executes the script on the user's selection.
 */
function perform()
{
	for(var it = localSession.getSelection(player.getWorld()).iterator(); it.hasNext(); )
	{
		var pt = it.next();
		var block = editSession.getBlock(pt);
		
		if(block.getType() == BlockID.COMMAND_BLOCK)
		{
			var nbt = new HashMap(block.getNbtData().getValue());
			
			var cmd = nbt.get("Command").getValue();
			var newCmd = translate(cmd, player);
			nbt.put("Command", new StringTag(newCmd));
			
			//Finalize changes to the world
			block.setNbtData(new CompoundTag(nbt));
			editSession.setBlock(pt, new BaseBlock(BlockID.BEDROCK)); //Workaround: force the client to acknowledge the block changed.
			editSession.setBlock(pt, block);
			player.print("Updated command block at " + pt.toString() + " from \"\247r" + cmd + "\247d\" to \"\247r" + newCmd + "\247d\".");
		}
		else if(block.getType() == BlockID.SIGN_POST || block.getType() == BlockID.WALL_SIGN)
		{
			var sign = new SignBlock(block.getType(), block.getData());
			sign.setNbtData(block.getNbtData());
			var text = sign.getText();
			
			var newText = new Array();
			for(var i = 0; i < 4; i++)
			{
				newText[i] = translate(text[i], player);
			}
			
			//Finalize changes to the world
			sign.setText(newText);
			editSession.setBlock(pt, new BaseBlock(BlockID.BEDROCK)); //Workaround: force the client to acknowledge the block changed.
			editSession.setBlock(pt, sign);
			
			player.print("Updated sign at " + pt.toString());
			for(var i = 0; i < 4; i++)
			{
				player.print("\2477Line " + (i+1) + ": " + text[i] + " to " + sign.getText()[i]);
			}
		}
	}
}
...
/**
 * Replaces all instances of text delimited by { or } with corresponding custom characters.
 * @param string - the text to search for character names to translate.
 * @returns The translated string
 */
function translate(string,player)
{
	var tokens = string.match(/([{][a-z0-9 ]*[}])/i);
	
	if(tokens != null)
		for(var i = 0; i < tokens.length; i++)
		{
			var charCode = characterNames.indexOf(tokens[i].toLowerCase().slice(1,-1));
			if(charCode >= 0)
			{
				string = addCharacter(string, tokens[i], charCode);
			}
			else
			{
				var alternate = -1;
				
				switch(String(tokens[i].toLowerCase().slice(1,-1))) //Note the conversion from Java String to JavaScript String. switch/case uses === operator.
				{
					case "compass":			alternate = characterNames.indexOf("compass north");			break;
					case "clock":			alternate = characterNames.indexOf("clock noon");				break;
					case "firework star":	alternate = characterNames.indexOf("firework star gray");		break;
					case "fire":			alternate = characterNames.indexOf("fire 0");					break;
					case "water":			alternate = characterNames.indexOf("stationary water");			break;
					case "lava":			alternate = characterNames.indexOf("stationary lava");			break;
					case "water solid":		alternate = characterNames.indexOf("stationary water solid");	break;
					case "cat":				alternate = characterNames.indexOf("ginger cat");				break;
					case "ender crystal":	alternate = characterNames.indexOf("ender crystal 1");			break;
					case "painting":		alternate = characterNames.indexOf("painting kebab");			break;
					case "xp orb":			alternate = characterNames.indexOf("xp orb 4");					break;
					case "health boost":	alternate = characterNames.indexOf("absorption");				break;
				}
				
				if(alternate >= 0)
					string = addCharacter(string, tokens[i], alternate);
				if(player != null)
					player.print("The token " + tokens[i] + " does not exist!");
			}
		}
...
function addCharacter(string, token, charCode)
{
	var replacement = Character.toString(unicodeOffset + charCode);
	if(escape)
		replacement = "\247f" + replacement + "\247r"; // \247 = section sign
